        ...
        ColumnLayout {
            ...
            RowLayout{
                ...
                IconImage {
                    ...
                    source: Quickshell.iconPath(modelData.desktopEntry)
                    implicitWidth: 30
                    implicitHeight: 30
                }
            ...
            CustomText {
                id: timeText
                property string progressTime: "0:00"
                text: progressTime
                Connections {
                    target: modelData
                    function onPositionChanged() {
                        var secs = Math.floor(modelData.position)
                        var min = Math.floor(secs / 60)
                        var sec = secs % 60
                        timeText.progressTime = min + ":" + (sec < 10 ? "0" : "") + sec
                    }
                }
                font.pixelSize: 12
                color: Theme.colors.foreground
